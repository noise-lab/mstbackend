...
def ping_servers():
    ...
    f = open(settings.CONFIG_PINGSERVERS, 'r')
    servers = {}
    servers['servers'] = {}
    for line in f:
	row = line.split('|')	
...
def get_countries():
    f = open(settings.CONFIG_COUNTRIES, 'r')   
    countries = []
    for line in f:
	line = line.rstrip('\n')
	country = {}
...
def dnsvalues():
    val = {}
    val['dns_targets'] = get_dnstargets()
    return val

def get_dnstargets():
    f = open(settings.CONFIG_DNSSERVERS, 'r')
    dnstargets = []
    for line in f:
      line = line.rstrip('\n')
      row = line.split('|')
      ...
      entry['server'] = row[0]
      entry['target'] = row[1]
      dnstargets.append(entry)
    f.close()
    return dnstargets

def surveysjson():
    f = open('/srv/mstwebapp/config/surveys.json', 'r')
    data = f.read()
    f.close()
    return data
